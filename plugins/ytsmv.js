...
`;
                ...
                const qualitySelectionHandler = async (messageUpdate2) => {
                    ...
                    if (userSelectedQuality && userSelectedQuality <= desc.torrents.length && userStates[from].currentSearchId === currentSearchId) {
                        ...
                        const magnetLink = `magnet:?xt=urn:btih:${selectedTorrent.hash}&dn=${encodeURIComponent(title)}&tr=udp://open.demonii.com:1337/announce&tr=udp://tracker.openbittorrent.com:80&tr=udp://tracker.coppersurfer.tk:6969`;

                        // Send the magnet link as a document with MIME type for video
                        await conn.sendMessage(from, {
                            document: { url: magnetLink },
                            mimetype: "video/x-matroska", // Change to "video/x-matroska" for MKV if needed
                            fileName: `${title}.mkv`,
